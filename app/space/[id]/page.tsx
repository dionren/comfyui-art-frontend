...
const spacesData = {
  ...
  "ai-character": {
    ...
    backgroundImage:
      "/placeholder.svg?height=400&width=1200&text=角色设计工作室&fontsize=32&textcolor=white&bgcolor=EC4899",
    author: {
      id: "user-2",
      name: "李设计",
      avatar: "/placeholder.svg?height=40&width=40&text=🎮&fontsize=16",
...
export default function SpacePage() {
  ...
  // 当前用户在此空间的订阅状态
  const [userSpaceSubscription, setUserSpaceSubscription] = useState<"free" | "regular" | "premium">("free")

  // 模拟获取用户在当前空间的订阅状态
  useEffect(() => {
    // 这里应该是从API获取数据
    // 模拟不同空间有不同的订阅状态
    if (spaceId === "advance-aigc") {
      setUserSpaceSubscription("regular")
    } else if (spaceId === "ai-character") {
      setUserSpaceSubscription("premium")
    } else {
      setUserSpaceSubscription("free")
    }
  }, [spaceId])

  // 获取当前空间数据
  const spaceData = spacesData[spaceId as keyof typeof spacesData] || {
    id: spaceId,
    name: "未知空间",
    description: "无法找到此空间的详细信息",
    members: 0,
    createdAt: new Date().toISOString(),
    updatedAt: new Date().toISOString(),
    backgroundImage: "/placeholder.svg?height=400&width=1200&text=未知空间&fontsize=32&textcolor=white&bgcolor=4F46E5",
    author: {
      id: "unknown",
      name: "未知作者",
      avatar: "/placeholder.svg?height=40&width=40&text=?&fontsize=16",
          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                ...
                >
                  {userSpaceSubscription === "premium" ? (
                    <Crown className="h-4 w-4 mr-2 text-amber-400" />
                  ) : userSpaceSubscription === "regular" ? (
        ...
        >
          ...
          >
            <div className="flex justify-between items-center mb-6">
              <h3 className="text-2xl font-bold text-white">"{spaceData.name}" 空间订阅计划</h3>
              <Button
                variant="ghost"
                size="sm"
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {subscriptionPlans.map((plan) => (
                ...
                >
                  ...
                  <Button
                    className={`w-full ${
                      plan.id === userSpaceSubscription
                        ? "bg-gray-700 hover:bg-gray-600 text-gray-300"
                        : plan.id === "premium"
                          ? "bg-amber-600 hover:bg-amber-700 text-white"
                          : "bg-purple-600 hover:bg-purple-700 text-white"
                    }`}
                    disabled={plan.id === userSpaceSubscription}
                  >
                    {plan.id === userSpaceSubscription ? "当前方案" : `升级至${plan.name}`}
                  </Button>
                </div>
              ))}
            </div>
            ...
            <div className="mt-8 bg-gray-800/50 rounded-lg p-4 border border-gray-700">
              ...
              <ul className="space-y-2 text-sm text-gray-300">
                ...
                <li className="flex items-start">
                  <span className="text-purple-400 mr-2">•</span>
                  <span>此订阅计划仅适用于"{spaceData.name}"空间，每个空间需单独订阅</span>
                </li>
                <li className="flex items-start">
                  <span className="text-purple-400 mr-2">•</span>
          ...
          <div className="container mx-auto px-4 h-full flex flex-col justify-end pb-8">
            <div className="flex flex-col">
              <div className="flex items-center mb-3">
                <h1 className="text-3xl font-bold text-white drop-shadow-md">{spaceData.name}</h1>
                <div className="ml-4 flex items-center bg-gray-800/70 backdrop-blur-sm rounded-full px-3 py-1.5">
                  <Avatar className="h-6 w-6 mr-2">
                    <AvatarImage src={spaceData.author.avatar || "/placeholder.svg"} alt={spaceData.author.name} />
                    <AvatarFallback className="bg-gray-700 text-gray-200">
                      {spaceData.author.name.charAt(0)}
                    </AvatarFallback>
                  </Avatar>
                  <span className="text-sm font-medium text-gray-200">{spaceData.author.name}</span>
                </div>
              </div>
              <p className="text-gray-300/90 drop-shadow-md max-w-2xl">{spaceData.description}</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-gray-900/60 backdrop-blur-sm rounded-xl p-4 -mt-6 mb-8 border border-gray-800/50 shadow-xl">
          <div className="flex flex-wrap items-center gap-4">
            ...
            >
              ...
              {formatNumber(spaceData.visits)} 访问量
            </Badge>
            <Badge
              variant="outline"
              className="text-sm py-1.5 bg-gray-800/50 text-gray-300 border-gray-700 hover:bg-gray-800"
            >
              ...
              更新于 {formatDate(spaceData.updatedAt)}
            </Badge>
